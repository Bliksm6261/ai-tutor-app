...
from fastapi.security import OAuth2PasswordBearer, OAuth2PasswordRequestForm # MODIFIED: Added for consistency
...
# MODIFIED: UserLogin is not needed if we use OAuth2PasswordRequestForm
...
# NEW: Dependency to get a user who is a teacher or an admin
...
app = FastAPI(
    ...
    version="0.9.1", # Version bump for disable feature
)
...
# MODIFIED: Updated /login endpoint with is_active checks
...
def login_for_access_token(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    try:
        ...
        # 2. NEW: Check if the user's account is active
        ...
        # 3. NEW: Fetch and check if the user's school is active
    ...
    # 4. If all checks pass, create and return the token
...
# ... (keep all your other endpoints like create_class, create_student, etc., the same)
...
# --- NEW: Endpoints for Disabling/Enabling Students ---
...
def disable_student(
    ...
    try:
        ...
        # Security check: Get school IDs for both teacher and student
        cursor.execute("SELECT school_id FROM users WHERE user_id = %s", (current_user.user_id,))
        teacher_record = cursor.fetchone()
...
def enable_student(
    ...
    try:
        ...
        # Security check (same as disable)
        cursor.execute("SELECT school_id FROM users WHERE user_id = %s", (current_user.user_id,))
        teacher_record = cursor.fetchone()
